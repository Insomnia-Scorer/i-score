            ...
            <div className="relative w-full max-w-lg bg-card/95 backdrop-blur-xl shadow-[0_20px_60px_rgba(0,0,0,0.15)] rounded-[32px] sm:rounded-[36px] overflow-hidden animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-8 duration-500 border border-border/50 flex flex-col max-h-[90vh]">
                ...
                <div className="relative z-10 text-left px-6 sm:px-8 pt-6 pb-4 flex items-start justify-between border-b border-border/50 shrink-0 bg-background/50">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl sm:text-2xl font-black tracking-tight text-foreground drop-shadow-sm mb-1">{opponent.name}</h2>
                ...
                <div className="relative z-10 px-6 sm:px-8 py-6 space-y-8 overflow-y-auto flex-1">
                    <div>
                        <h3 className="text-sm font-black text-muted-foreground uppercase tracking-widest flex items-center gap-2 mb-3"><Swords className="h-4 w-4 text-primary/70" /> 通算対戦成績</h3>
                    ...
                    <div>
                        <h3 className="text-sm font-black text-muted-foreground uppercase tracking-widest flex items-center gap-2 mb-3"><CalendarDays className="h-4 w-4 text-primary/70" /> 直近の対戦履歴</h3>
                        <div className="space-y-3">
                            {opponent.recentMatches.map((match) => (
                                <div key={match.id} className="bg-muted/30 border border-border/50 rounded-[20px] p-4 hover:bg-muted/50 transition-colors cursor-pointer shadow-sm">
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        <span className={cn("text-[10px] font-black px-2 py-0.5 rounded-full border uppercase tracking-widest", getResultColor(match.result))}>{getResultText(match.result)}</span>
                                    </div>
                                    <div className="flex items-center justify-between mt-3">
                                        ...
                                        <div className="px-4 text-2xl font-black font-mono tracking-tighter">{match.myScore} - {match.opponentScore}</div>
