...
import { Plus, History, Trophy, Calendar, ChevronRight, MapPin, Loader2, Clock } from "lucide-react";
...
export default function DashboardPage() {
  ...
  // 権限チェック
  const isApproved = isApprovedMember(userRole);
  const canEdit = canEditScore(userRole);

  useEffect(() => {
    // 承認されていないユーザーは API を叩かないように防御
    if (!isApproved) {
      setIsLoading(false);
      return;
    }
  ...
  if (!isApproved) {
    return (
      <div className="container mx-auto max-w-2xl px-4 py-16 flex flex-col items-center justify-center text-center animate-in fade-in duration-500">
        <div className="h-24 w-24 bg-muted rounded-full flex items-center justify-center mb-6 shadow-inner">
          <Clock className="h-10 w-10 text-muted-foreground" />
        </div>
        <h1 className="text-3xl font-extrabold tracking-tight mb-4">承認待ちです</h1>
        <p className="text-muted-foreground mb-8 leading-relaxed">
          アカウントの登録が完了しました。<br />
          チームのデータにアクセスするには、管理者（IT担当・監督）による権限の割り当てが必要です。承認されるまでしばらくお待ちください。
        </p>
        <p className="text-sm font-bold text-primary bg-primary/10 px-4 py-2 rounded-full">
          現在のステータス: 仮登録 (Pending)
        </p>
      </div>
    );
  }

  // 💡 ここから下は承認済みユーザー向けの通常のダッシュボード
  return (
    <div className="container mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8 space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-xs text-muted-foreground">
          現在の権限: <span className="font-bold text-primary">{userRole}</span>
        </p>
      </div>
      ...
      <div className="space-y-6">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 bg-muted/30 rounded-2xl border border-dashed border-border">
            ...
            {canEdit && <Button asChild variant="outline"><Link href="/matches/new">最初の試合を記録する</Link></Button>}
          </div>
        ) : (
